...
export default function Campaigns() {
  ...
  // Campaigns user is in (joined or created)
  ...
  const availableCampaigns = allCampaigns.filter(c => !myCampaignIds.has(c.id));
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <div className={styles.list}>
          {myCampaigns.length === 0 ? (
            ...
            myCampaigns.map((c: Campaign) => <CampaignCard key={c.id} campaign={c} />)
          )}
        </div>
      </section>
      <section>
        ...
        <div className={styles.list}>
          {availableCampaigns.length === 0 ? (
            ...
            availableCampaigns.map((c: Campaign) => <CampaignCard key={c.id} campaign={c} />)
          )}
        </div>
      </section>
    </div>
