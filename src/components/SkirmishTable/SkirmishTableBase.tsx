        ...
        {skirmishes.map(sk => {
          ...
          return (
            <React.Fragment key={key}>
              {renderRow(sk, expanded, () => setExpandedRows(rows => ({ ...rows, [key]: !rows[key] })))}
              {expanded && renderExpanded && (
                <tr className={styles['skirmish-table__row']}>
                  <td colSpan={columns.length + 1} className={styles['skirmish-table__expanded']}>
                    <div className={styles['skirmish-table__expanded-actions']}>
                      {renderExpanded(sk)}
                    </div>
                  </td>
                </tr>
