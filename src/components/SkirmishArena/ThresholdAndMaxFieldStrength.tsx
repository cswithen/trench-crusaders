...
function getBattleInfo(completedMatches: number) {
    ...
    return (
        battleData.find((b) => b.battle === battleNum) ??
        battleData[battleData.length - 1]
    );
}

export default function ThresholdAndMaxFieldStrength({
    completedMatches,
}: {
    ...
    if (completedMatches === undefined || completedMatches === null) {
        return (
            <div className="threshold-fields">
                ...
                <div
                    style={{ fontSize: '0.95em', color: '#888', marginTop: 4 }}
                >
                    <span
                        tabIndex={0}
